...
export function ShortlistClient() {
  ...
  if (loading) {
    return <div className="rounded-[2rem] border border-slate-200 bg-white p-8 text-zinc-600 shadow-[0_18px_60px_rgba(15,23,42,0.08)]">Loading shortlist...</div>;
  }

  if (items.length === 0) {
    return (
      <div className="rounded-[2rem] border border-dashed border-slate-200 bg-white p-10 text-center shadow-[0_18px_60px_rgba(15,23,42,0.08)] backdrop-blur-xl">
        <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-rose-500">
          <BookmarkX className="h-7 w-7" />
        </div>
        <h2 className="mt-5 text-2xl font-semibold text-zinc-900">Your shortlist is empty.</h2>
        ...
        <Link href="/" className="mt-6 inline-flex rounded-full bg-zinc-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-zinc-800">
          Browse colleges
        </Link>
      </div>
    );
    ...
    <div className="space-y-6 text-zinc-900">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-[2rem] border border-slate-200 bg-white p-5 shadow-[0_18px_60px_rgba(15,23,42,0.08)] backdrop-blur-xl">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold text-zinc-900">Shortlisted Colleges</h1>
          <p className="mt-2 text-zinc-600">
            {source === 'database' ? 'Saved in the database for this browser profile.' : 'Loaded from local storage fallback.'}
          </p>
        </div>
        <Link href="/" className="rounded-full border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-zinc-700 transition hover:border-slate-300 hover:bg-slate-50 hover:text-zinc-900">
          Add more
        </Link>
      </div>
